...
SYSCALL_DEFINE2(sh_task_info, long, pid, char*, path) {
	...
	for_each_process(proces) {
		if(pid == (long)task_tgid_nr(proces)){
			t =1;
			// printk(
			// "Process: %s\n \
			// PID_Number: %ld\n \
			...
			// sprintf(a, "PID_Number: %ld \n", (long)task_pid_nr(proces));
			// sprintf(b, "Process State: %ld\n", (long)proces->state);
			// sprintf(c, "Priority: %ld\n", (long)proces->prio);
			// sprintf(d, "RT_Priority: %ld\n", (long)proces->rt_priority);
			// sprintf(e, "Static Priority: %ld\n", (long)proces->static_prio);
			// sprintf(f, "Normal Priority: %ld\n", (long)proces->normal_prio);
			...
			// sprintf(data,"PID_Number: %ld \n Process State: %ld \n Priority: %ld \n RT_Priority: %ld \n Static Priority: %ld \n Normal Priority: %ld\n \0", (long)task_pid_nr(proces),  (long)proces->state, (long)proces->prio, (long)proces->static_prio, (long)proces->normal_prio);

			printk(KERN_INFO "PID_Number: %ld \n Task state: %d \n CPU number : %d \n last : %d \n exit : %d \n Priority: %d \n", (long)task_pid_nr(proces), proces->state,  proces->on_cpu, proces->recent_used_cpu, proces->exit_state, proces->prio);
    		sprintf(data,"PID_Number: %ld \n Task state: %d \n CPU number : %d \n last : %d \n exit : %d \n Priority: %d \n \0", (long)task_pid_nr(proces), proces->state,  proces->on_cpu, proces->recent_used_cpu, proces->exit_state, proces->prio);
			long copied = strncpy_from_user(buff, path, sizeof(buff));
			// if(copied!=0){
			// 	printk("Error copying file");
			// 	return -EFAULT;
			// }
			int b = strlen(buff);
			...
			fil = filp_open(buff,O_CREAT, y);
			if(fil<0){
				return -1;
			}
			filp_close(fil, NULL);
			y =0;
			fil =  filp_open(buff,O_WRONLY, y);

			if(fil<0){
				return -1;
			}

			long var = kernel_write(fil, data, 100 , 0);
			if(var<0){
				return -1;
			}
